...
def classify_product(title):
    t = title.lower()
    if any(k in t for k in ['摆件', '手办', '桌面', '客厅', '玄关', '书房', '茶宠']):
        return '摆件/手办'
    if any(k in t for k in ['手串', '隔珠', '隔片', '三通', '背云', '链接扣', '卡环', '跑环', '配珠', '散珠', '合集']):
        return '手串配件'
    if any(k in t for k in ['吊坠', '项链', '挂坠', '挂件', '绳链']):
        return '吊坠/挂件'
    if any(k in t for k in ['手把件', '解压', 'edc']):
        return '手把件/EDC'
    if any(k in t for k in ['葫芦', '貔貅', '龙', '凤', '神兽', '瑞兽', '霸下', '玄武', '龙龟']):
        return '国风摆件'
    return '其他'
...
def main():
    ...
    report_lines.append("琢匠店铺 MHTML 样本分析 Report")
    report_lines.append("=" * 60)
